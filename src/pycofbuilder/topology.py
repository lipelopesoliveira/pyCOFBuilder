...
class Topology:
    ...
    def __repr__(self) -> str:
        return "Topology(name={}, spacegroup={}, nodes={}, edge_centers={})".format(
            self.name, self.spacegroup, len(self.nodes), len(self.edge_centers)
        )
    ...
    def __str__(self) -> str:
        ...
        topo_str += (
            "Cell: {:10.5f} {:10.5f} {:10.5f} {:10.5f} {:10.5f} {:10.5f}\n".format(
                *self.cell.cellpar()
            )
        )
    ...
    def read_cgd_file(
        self, cgd_file: str | Path, expand_by_symmetry: bool = True
    ) -> None:
        """
        ...
        self.edge_centers = [
            EdgeCenter(*edge_center) for edge_center in parsed_file["edge_centers"]
        ]

    @classmethod
    def from_cgd_file(
        cls, cgd_file: str | Path, expand_by_symmetry: bool = True
    ) -> "Topology":
        """
        Parses a CGD file to populate the topology's nodes and edge centers
        """
    ...
    def to_cgd(self, output_file: str | Path) -> None:
        ...
        gcd_file = [
            ...
            f"NAME {self.name}",
            "GROUP P1",
            "CELL {:<10.5f} {:<10.5f} {:<10.5f} {:<10.5f} {:<10.5f} {:<10.5f}".format(
                *self.cell.cellpar()
            ),
        ]
        ...
        for node in self.nodes:
            gcd_file.append(
                "NODE {:2} {:2} {:<10.5f} {:<10.5f} {:<10.5f}".format(
                    node.id, node.connectivity, node.x, node.y, node.z
                )
            )
        for edge_center in self.edge_centers:
            gcd_file.append(
                "EDGE_CENTER {:<10.5f} {:<10.5f} {:<10.5f}".format(
                    edge_center.x, edge_center.y, edge_center.z
                )
            )
