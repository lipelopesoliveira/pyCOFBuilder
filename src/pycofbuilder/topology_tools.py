...
def expand_topology_by_symmetry(parsed_data: dict) -> dict:
    ...
    species = ["C"] * len(parsed_data["nodes"]) + ["H"] * len(
        parsed_data["edge_centers"]
    )
    positions = [
        tuple([node["x"], node["y"], node["z"]]) for node in parsed_data["nodes"]
    ] + parsed_data["edge_centers"]
    properties = {
        "connectivity": [node["connectivity"] for node in parsed_data["nodes"]]
        + [2 for _ in parsed_data["edge_centers"]],
        "id": [node["id"] for node in parsed_data["nodes"]]
        + [-i - 1 for i in range(len(parsed_data["edge_centers"]))],
        "kind": ["node"] * len(parsed_data["nodes"])
        + ["edge_center"] * len(parsed_data["edge_centers"]),
    }
    ...
    expandded_data = {
        ...
        "nodes": [
            ...
            if site.properties["kind"] == "node"
        ],
        "edge_centers": [
            site.frac_coords.tolist()
            for site in structure
            if site.properties["kind"] == "edge_center"
        ],
    }
        ...
        parsed_data["edges"].append(
            {
                "start": (
                    ...
                    parse_val(match.group(2)),
                    parse_val(match.group(3)),
                ),
                "end": (
                    parse_val(match.group(4)),
                    parse_val(match.group(5)),
                    parse_val(match.group(6)),
                ),
            }
        )
